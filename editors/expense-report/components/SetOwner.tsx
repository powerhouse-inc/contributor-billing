...
export function SetOwner({ ownerId, dispatch }: SetOwnerProps) {
  ...
  const { ownerDriveId, ownerId: resolvedOwnerId, selectedDocument } =
...
function OwnerDriveActions({
  ...
  useEffect(() => {
    ...
    setAddStatus("idle");
  }, [isAlreadyAdded, selectedDocumentId, expenseReportNodeIds, ownerDriveId, driveDocumentId]);
  ...
  return (
    ...
    >
      {addStatus === "adding" ? "Adding report..." : `Add Report to ${ownerName}`}
    </button>
  );
}