...
export class WalletAccountService {
  ...
  async processWalletAddition(
    ...
    allDocuments: Array<{ header: { id: string; documentType: string } }> | undefined
    ...
    try {
      ...
      console.log('[WalletAccountService] Step 1: Finding or creating Accounts document', {
        driveId,
      ...
      if (!driveId) {
        console.warn('[WalletAccountService] No driveId provided - documents will be created but not added to drive');
      }
      ...
      console.log('[WalletAccountService] Accounts document result:', accountsDocResult);

      if (!accountsDocResult.success || !accountsDocResult.documentId) {
        const errorMsg = accountsDocResult.message || "Failed to find or create Accounts document";
      ...
      if (driveId) {
        console.log('[WalletAccountService] Waiting for document to be added to drive...');
      ...
      if (!accountResult.success || !accountResult.accountId) {
        const errorMsg = accountResult.message || "Failed to find or add account";
      ...
      console.log('[WalletAccountService] Step 3: Handling Account Transactions document', {
        hasExistingTxnsDoc: !!(account.accountTransactionsId && account.accountTransactionsId !== null && account.accountTransactionsId !== ""),
        driveId
      });
      let accountTransactionsDocumentId: string | undefined;

      if (account.accountTransactionsId && account.accountTransactionsId !== null && account.accountTransactionsId !== "") {
        // Account Transactions document already exists, update it
        console.log('[WalletAccountService] Account Transactions document exists, updating:', account.accountTransactionsId);
        accountTransactionsDocumentId = account.accountTransactionsId;
        await this.updateAccountTransactions(accountTransactionsDocumentId, walletAddress);
      } else {
        // Create new Account Transactions document
        console.log('[WalletAccountService] Creating new Account Transactions document with driveId:', driveId);
        const txnsResult = await accountTransactionsService.createAccountTransactionsDocument(
          account,
          accountsDocumentId,
          driveId
        );

        console.log('[WalletAccountService] Account Transactions creation result:', txnsResult);

        if (txnsResult.success && txnsResult.documentId) {
          accountTransactionsDocumentId = txnsResult.documentId;
          // Wait for document to be added to drive and processed
          if (driveId) {
            console.log('[WalletAccountService] Waiting for Account Transactions document to be added to drive...');
            // Wait longer to ensure the drive state is updated and UI can refresh
            await new Promise(resolve => setTimeout(resolve, 1000));
          }
        } else {
          console.warn('[WalletAccountService] Failed to create Account Transactions document:', txnsResult.message);
        }
      }
      ...
      console.log('[WalletAccountService] Wallet addition process completed successfully:', result);
      return result;
    } catch (error) {
      const errorMsg = `Error processing wallet addition: ${error instanceof Error ? error.message : 'Unknown error'}`;
      console.error('[WalletAccountService] Error in processWalletAddition:', error);
      return {
        success: false,
        message: errorMsg
      };
  ...
  private async findOrCreateAccountsDocument(
    ...
    if (allDocuments && driveId) {
      ...
      console.log('[WalletAccountService] Cannot search for existing document:', {
        hasDocuments: !!allDocuments,
        hasDriveId: !!driveId
      });
    }
    ...
    try {
      ...
      console.log('[WalletAccountService] Sending GraphQL request with variables:', variables);
      ...
      console.log('[WalletAccountService] GraphQL request body:', JSON.stringify(requestBody, null, 2));
      ...
      console.log('[WalletAccountService] Create Accounts document response status:', response.status);
      ...
      console.log('[WalletAccountService] Create Accounts document result:', result);
      ...
      if (!documentId) {
        throw new Error("Failed to create Accounts document - no document ID returned");
      }

      console.log('[WalletAccountService] Successfully created Accounts document:', documentId);
      ...
      console.error('[WalletAccountService] Error creating Accounts document:', error);
      return {
        success: false,
        message: errorMsg
      };
  ...
  private async findOrAddAccount(
    ...
    try {
      ...
      while (retries > 0) {
        try {
          ...
          if (!getDocResponse.ok) {
            throw new Error(`Failed to get Accounts document: ${getDocResponse.status}`);
          }
          ...
          if (getDocResult.errors) {
            const errorMsg = getDocResult.errors[0]?.message || 'Failed to get Accounts document';
            // If document not found and we have retries left, wait and retry
            if (errorMsg.includes('not found') && retries > 1) {
              console.log(`[WalletAccountService] Document not found, retrying in 500ms... (${retries - 1} retries left)`);
          ...
          accounts = getDocResult.data?.Accounts?.getDocument?.state?.accounts || [];
          break; // Success, exit retry loop
        } catch (error) {
          lastError = error instanceof Error ? error : new Error(String(error));
          if (retries > 1) {
            console.log(`[WalletAccountService] Error querying document, retrying in 500ms... (${retries - 1} retries left)`, lastError.message);
            await new Promise(resolve => setTimeout(resolve, 500));
            retries--;
          } else {
            throw lastError;
      ...
      if (existingAccount) {
        console.log('[WalletAccountService] Account already exists:', existingAccount.id);
        return {
          success: true,
          ...
          account: {
            ...
            budgetPath: null,
            accountTransactionsId: existingAccount.accountTransactionsId || null,
            chain: ["ethereum"],
            type: null,
            owners: [],
            KycAmlStatus: null
      ...
      console.log('[WalletAccountService] Account does not exist, adding new account');
      ...
      if (!addAccountResponse.ok) {
        ...
        throw new Error(`Failed to add account: ${addAccountResponse.status} - ${errorText}`);
      }
      ...
      if (addAccountResult.errors) {
        throw new Error(addAccountResult.errors[0]?.message || 'Failed to add account');
      }

      // Ensure the mutation reports success
      if (!addAccountResult.data?.Accounts_addAccount) {
        throw new Error('Failed to add account - mutation returned falsy result');
      }

      console.log('[WalletAccountService] Account added successfully:', accountId);
  ...
  private async updateAccountTransactions(
    ...
    try {
      await accountTransactionsService.syncTransactionsForDocument(documentId, address);
    } catch (error) {
