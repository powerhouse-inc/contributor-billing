...
export function LineItemTagsTable({
  ...
  const [paymentAccount, setPaymentAccount] = useState<string>("Powerhouse USD");
    ...
    <div className="w-full">
              ...
              <tr key={item.id} className="hover:bg-gray-50">
                <td className="border-b border-gray-200 p-3">
                  <input
                    ...
                    value={item.item}
                    onChange={(e) =>
                      setTaggedItems(prev => prev.map(row => row.id === item.id ? { ...row, item: e.target.value } : row))
                    }
                ...
                <td className="border-b border-gray-200 p-3">
                  <select
                    value={item.period}
                    onChange={(e) =>
                      setTaggedItems(prev => prev.map(row => row.id === item.id ? { ...row, period: e.target.value } : row))
                    }
                    className="w-full rounded border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none"
                  >
                ...
                <td className="border-b border-gray-200 p-3">
                  <select
                    ...
                    onChange={(e) =>
                      setTaggedItems(prev => prev.map(row => row.id === item.id ? { ...row, expenseAccount: e.target.value } : row))
                    }
                    className="w-full rounded border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none"
                  >
      ...
      <div className="border-t border-gray-200 p-6">
        <div className="flex items-center justify-end gap-4">
          <label className="text-lg font-medium text-gray-900">Payment Account</label>
          <select
