...
- **MCP Server**: Must be running at `http://localhost:4001/mcp`
- **Start Command**: Ask user to run `ph vetra` or `bun run reactor` if unavailable
- **DO NOT** start the MCP server yourself - always ask the user

### Communication Requirements

- **Always use HTTP POST** with JSON-RPC 2.0 protocol
- **Required Headers**:
  ```
  Content-Type: application/json
  Accept: application/json, text/event-stream
  ```
- **Parameter Names**: Use exact parameter names from tool schema (e.g., `id` not `documentId` for getDocument)
- **Response Format**: Parse SSE format responses (lines starting with `event:` and `data:`)
...
```
event: message
data: {"jsonrpc": "2.0", "id": 1, "error": {"code": -32602, "message": "..."}}
```

### Common MCP Tools and Parameter Mappings

**Document Operations:**
- `getDocument`: requires `driveId` (string), `id` (string) - NOT `documentId`
- `createDocument`: requires `name` (string), `driveId` (string, optional)
- `addActions`: requires `driveId` (string), `documentId` (string), `actions` (array)
  - Each action needs: `type` (string), `scope` (string), `input` (object)

**Drive Operations:**
- `getDrive`: requires `driveId` (string)
- `getDrives`: no parameters required
- `getDocuments`: requires `driveId` (string), `parentId` (string)

**Document Model Operations:**
- `getDocumentModels`: no parameters required
- `getDocumentModelSchema`: requires `type` (string) - NOT `documentType`
...
1. **"Not Acceptable: Client must accept both application/json and text/event-stream"**
   - Solution: Include both `Content-Type: application/json` AND `Accept: application/json, text/event-stream` headers

2. **"Invalid arguments for tool X: Required parameter 'Y'"**
   - Solution: Check tool documentation for exact parameter names (e.g., use `id` not `documentId` for getDocument)

3. **MCP server unavailable**
   - Solution: Ask user to run `ph vetra` or `bun run reactor` in a separate terminal
