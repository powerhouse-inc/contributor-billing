 ...
 */
export function autoTagLineItem(description: string, amount: number, vendor?: string): TaggingResult | null {
 ...
 */
export async function claudeClassifyExpense(description: string, amount: number): Promise<TaggingResult | null> {
...
Respond with ONLY the category name: SOFTWARE|TRAVEL|CONTRACTOR|LEGAL|OFFICE|MARKETING|OTHER`;
  ...
  try {
    ...
    const account = categoryMapping[category as keyof typeof categoryMapping] || '453';
...
export async function autoTagLineItems(lineItems: any[]): Promise<any[]> {
  ...
  for (const item of lineItems) {
    ...
    if (result && result.confidence >= 0.7) {
      ...
      const claudeResult = await claudeClassifyExpense(item.description, item.totalPriceTaxIncl);
      if (claudeResult) {
